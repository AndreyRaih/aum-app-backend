...
function normalizePose(boundingBox, keypoints) {
    ...
    for (let i = 0; i < newKeypoints.length; i++) {
        newKeypoints[i].position.x = l2normalize(newKeypoints[i].position.x, boundingBox.maxX, boundingBox.minX);
        newKeypoints[i].position.y = l2normalize(newKeypoints[i].position.y, boundingBox.maxY, boundingBox.minY);
    }
    return keypoints;
}
/**
...
//# sourceMappingURL=pose-experimental-compares.js.map