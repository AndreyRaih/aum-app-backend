...
exports.getQueueFromFirebase = (blocks) => repository.getAsanaList(blocks);
...
exports.parseResultsForUpdates = async ({ id, estimations }) => {
    const user = await repository.getUserModel(id);
    const updates = utils_1.buildResultObject(user, estimations);
    return {
        id,
        updates
    };
...
exports.addNewSession = async (id, sessionData) => {
    ...
    sessions.push(Object.assign({ id: '', date: utils_1.formatDateToString(new Date()), duration: sessionData.asanaQuantity * 45, cal: sessionData.asanaQuantity * 10 }, sessionData));
    return { sessions };
};
exports.createFact = () => {
